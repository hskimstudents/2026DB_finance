...
# 시계열 특성 유지하기 위해 먼저 시퀀스 만들고 나서 split 해도 되지만,
# 여기서는 스케일러를 Train 구간 기준으로 fit 하기 위해 먼저 인덱스 기준 split
...
# Feature, Target 각각 MinMaxScaling
...
# (시퀀스 만들면서 앞 WINDOW_SIZE일이 날아갔기 때문에 그만큼 보정)
...
# 1) 전체 모델(구조+가중치+optimizer 상태) 저장
...
# 2) 가중치만 따로 저장 (나중에 같은 구조의 모델에 로드 가능)
# model.save_weights("lstm_stock_weights_only.h5")
# print("가중치만 저장: lstm_stock_weights_only.h5")
import matplotlib.pyplot as plt
import matplotlib.dates as mdates
import numpy as np

# =========================
# 1️⃣ 테스트 구간 실제 종가 & 날짜 가져오기
# =========================

# 시퀀스 만들면서 앞에 WINDOW_SIZE만큼 잘렸으니까
# 테스트 구간의 시작 인덱스는 이렇게 계산
test_start_idx = WINDOW_SIZE + sequence_train_size

# 테스트 구간의 실제 종가 (Actual Price)
actual_prices = df["종가"].values[test_start_idx:]          # shape: (len(y_test),)
test_dates    = df["날짜"].values[test_start_idx:]          # 같은 길이의 날짜 배열

print("actual_prices len:", len(actual_prices))
print("y_pred_inv len   :", len(y_pred_inv))

# =========================
# 2️⃣ 예측 수익률 -> 예측 종가 재구성 (시작점 맞추기)
# =========================
...
predicted_prices = start_price * cum_returns                # 첫 값이 actual과 동일하게 시작

# =========================
# 3️⃣ 그래프 그리기 (날짜 축 + 예측선 살짝 내려간 효과)
# =========================
...
# x축을 날짜 형식으로 보기 좋게 표시
ax = plt.gca()
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.gcf().autofmt_xdate()   # 날짜 라벨 기울여서 겹치지 않게
